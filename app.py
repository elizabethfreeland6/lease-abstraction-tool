...
from utils.history_manager import save_extraction, load_extraction, list_extractions, delete_extraction, clear_all_history, get_extraction_count
...
def main():
    ...
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📤 Upload & Process", "✏️ Review & Edit", "📊 Export", "📜 History"])
...
def process_documents(uploaded_files):
    ...
    if all_extracted_data:
        ...
        for doc in all_extracted_data:
            try:
                save_extraction(doc['filename'], doc['data'])
            except Exception as e:
                st.warning(f"Could not save {doc['filename']} to history: {str(e)}")
...
def history_tab():
    st.markdown('<div class="sub-header">📜 Previous Lease Extractions</div>', unsafe_allow_html=True)
    
    # Get extraction count
    count = get_extraction_count()
    
    if count == 0:
        st.info("💭 No saved extractions yet. Process some leases to see them here!")
        return
    
    st.markdown(f"**Total saved extractions:** {count}")
    
    # Search box
    search_term = st.text_input("🔍 Search by property, tenant, or filename", placeholder="Enter search term...")
    
    # Get extractions
    extractions = list_extractions(search_term)
    
    if not extractions:
        st.warning("⚠️ No extractions found matching your search.")
        return
    ...
    for extraction in extractions:
        with st.container():
            ...
            with col1:
                st.markdown(f"### 📄 {extraction['filename']}")
                
                # Display key info
                info_cols = st.columns(3)
                with info_cols[0]:
                    st.markdown(f"**Property:** {extraction.get('property_address', 'N/A')}")
                with info_cols[1]:
                    st.markdown(f"**Tenant:** {extraction.get('tenant_name', 'N/A')}")
                with info_cols[2]:
                    st.markdown(f"**Lease Start:** {extraction.get('lease_start_date', 'N/A')}")
                
                st.caption(f"🕒 Processed: {extraction['timestamp']}")
            
            with col2:
                # Action buttons
                if st.button("👁️ View", key=f"view_{extraction['id']}", use_container_width=True):
                    st.session_state[f"show_details_{extraction['id']}"] = not st.session_state.get(f"show_details_{extraction['id']}", False)
                
                if st.button("📝 Load", key=f"load_{extraction['id']}", use_container_width=True):
                    # Load extraction into session state
                    full_extraction = load_extraction(extraction['id'])
                    if full_extraction:
                        st.session_state.extracted_data = [{
                            'filename': full_extraction['filename'],
                            'data': full_extraction['data'],
                            'extracted_at': full_extraction['timestamp']
                        }]
                        st.session_state.processing_complete = True
                        st.success("✅ Loaded! Go to 'Review & Edit' tab to view/edit.")
                        st.rerun()
                
                if st.button("🗑️ Delete", key=f"delete_{extraction['id']}", use_container_width=True):
                    if delete_extraction(extraction['id']):
                        st.success(f"✅ Deleted {extraction['filename']}")
                        st.rerun()
            
            # Show details if toggled
            if st.session_state.get(f"show_details_{extraction['id']}", False):
                with st.expander("📊 Full Details", expanded=True):
                    ...
                    if full_extraction:
                        ...
                        st.markdown("#### Tenant Information")
                        st.write(f"**Name:** {data.get('tenant_name', 'N/A')}")
                        st.write(f"**Email:** {data.get('tenant_email', 'N/A')}")
                        st.write(f"**Phone:** {data.get('tenant_phone', 'N/A')}")
                        
                        st.markdown("#### Property Information")
                        st.write(f"**Address:** {data.get('property_address', 'N/A')}")
                        st.write(f"**Unit:** {data.get('unit_number', 'N/A')}")
                        st.write(f"**Type:** {data.get('property_type', 'N/A')}")
                        st.write(f"**Square Footage:** {data.get('square_footage', 'N/A')}")
                        
                        st.markdown("#### Lease Terms")
                        st.write(f"**Start Date:** {data.get('lease_start_date', 'N/A')}")
                        st.write(f"**End Date:** {data.get('lease_end_date', 'N/A')}")
                        st.write(f"**Term:** {data.get('lease_term_months', 'N/A')} months")
                        st.write(f"**Type:** {data.get('lease_type', 'N/A')}")
                        
                        st.markdown("#### Financial Terms")
                        st.write(f"**Monthly Rent:** ${data.get('monthly_rent', 0):.2f}")
                        st.write(f"**Security Deposit:** ${data.get('security_deposit', 0):.2f}")
                        st.write(f"**Late Fee Type:** {data.get('late_fee_type', 'N/A')}")
                        if data.get('late_fee_type') == 'percentage':
                            st.write(f"**Late Fee:** {data.get('late_fee_percentage', 0)}%")
                        elif data.get('late_fee_type') == 'flat_amount':
                            st.write(f"**Late Fee:** ${data.get('late_fee_flat_amount', 0):.2f}")
    ...
    if st.session_state.get('confirm_clear', False):
        st.warning("⚠️ Are you sure you want to delete ALL saved extractions? This cannot be undone!")
